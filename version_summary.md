...
前期通过人工审核方法标签和变量标签的质量，发现claude-opus-4.6效果最好。抽查了 35 条样本（来自不同教材），在方法打标方面均识别正确；在变量抽取方面，也仅有 1 个问题。因此，后续默认使用 claude-opus-4.6 作为主模型。

在得到llm给出的方法审阅结果之后，观察“proposed_new_category“字段的非空取值，我们又更新了一版的 taxonomy，并基于新的 taxonomy 对方法标签做了更新。

> 之前尝试了利用gpt5.4等模型给出的预测结果作为reference，让 claude重新审视自己的结果，但发现用处不大，因此后面没有采用这种多llm互审的方式。
